...
import { createHederaTxSender, createEvmTxSender } from "./tx";
...
async function connectHederaWallet(chain: ChainConfig): Promise<string> {
  ...
  // Static import - no dynamic chunk loading, no ChunkLoadError.
  // HashConnect is bundled upfront via the static import at the top.
  const HashConnectClass = HashConnect;
...
function hederaGetTxSender(chain: ChainConfig): () => Promise<TxSender> {
  return async () => {
    ...
    // Static import - bundled upfront, no chunk loading
